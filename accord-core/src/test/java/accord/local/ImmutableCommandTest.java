...
public class ImmutableCommandTest
{
    ...
    void noConflictWitnessTest()
    {
        ...
        commands.execute(() -> {
            SafeCommandStore safeStore = commands.beginOperation(ExecutionContext.contextFor(txnId, "Test"), null);
            try
            {
    ...
    void supersedingEpochWitnessTest() throws ExecutionException
    {
        ...
        ExecutionContext context = ExecutionContext.contextFor(txnId, "Test");
        ...
        commands.chain(ExecutionContext.contextFor(txnId, "Test"), safeStore -> {
            Command command = safeStore.get(txnId).current();
            Assertions.assertEquals(Status.PreAccepted, command.status());
