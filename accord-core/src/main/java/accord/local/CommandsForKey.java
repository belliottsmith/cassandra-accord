...
public class CommandsForKey extends CommandsForKeyUpdate implements CommandsSummary
{
    ...
    // TODO (desired): avoid loading if we know a transaction that should have witnessed it has already been registered/applied
    ...
    private CommandsForKeyUpdate notifyUnmanaged(@Nullable TxnInfo curInfo, TxnInfo newInfo)
    {
        ...
        Unmanaged[] unmanageds = this.unmanageds;
        {
            ...
            int end = findCommit(unmanageds, minUndecidedById < 0 ? Timestamp.MAX : byId[minUndecidedById]);
            if (end > 0)
            {
        ...
        {
            TxnInfo maxContiguousAnyApplied = maxContiguousApplied();
            if (maxContiguousAnyApplied != null && maxContiguousAnyApplied.compareExecuteAt(newInfo) < 0)
                maxContiguousAnyApplied = null;

            if (maxContiguousAnyApplied != null)
            {
                int start = findFirstApply(unmanageds);
                int end = findApply(unmanageds, start, maxContiguousAnyApplied.executeAt);
                if (start != end)
                {
        ...
        else if (newInfo.status == INVALID_OR_TRUNCATED && curInfo != null && curInfo.status.isCommitted())
        {
            ...
            if (start != end)
            {
                ...
                if (predecessor >= 0)
                {
                    int maxContiguousAppliedAny = maxContiguousAnyAppliedIndex();
    ...
    private CommandsForKey pruneBefore(TxnInfo newPrunedBefore, int pos)
    {
        ...
        TxnInfo[] newInfos;
        {
            ...
            if (minUndecidedById < 0) // must occur later, so deduct all removals
    ...
    private int maxContiguousAnyAppliedIndex()
    {
    ...
    private TxnInfo maxContiguousApplied()
    {
        int i = maxContiguousAnyAppliedIndex();
