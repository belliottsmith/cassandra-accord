...
public interface CommandSummaries
{
    ...
    enum Relevance
    {
        ...
        public static final int ENCODED_BITS = 2;

        Relevance(int encoded)
        {
            Invariants.require(encoded == ordinal());
        }

        public boolean is(Relevance relevance)
        {
            return (relevance.ordinal() & ordinal()) == relevance.ordinal();
        }

        public Relevance or(Relevance that)
        {
            return forOrdinal(this.ordinal() | that.ordinal());
        }
    ...
    class Summary extends TxnId
    {
        ...
        public Summary(@Nonnull TxnId txnId, @Nullable Timestamp executeAt, @Nonnull SummaryStatus status, @Nonnull Durability durability, IsDep dep, Relevance relevance, Unseekables<?> participants)
        {
            ...
            this.encoded = Invariants.nonNull(status).ordinal()
                           ...
                           | (relevance.ordinal() << RELEVANCE_SHIFT);
        }
        ...
        public boolean is(Relevance relevance)
        {
            return ((encoded >> RELEVANCE_SHIFT) & relevance.ordinal()) == relevance.ordinal();
        }
    ...
    class SummaryLoader
    {
        ...
        {
            return relevance(txnId, null, null, null, null) != IRRELEVANT;
        }

        public final Relevance relevance(Command cmd)
        {
            return relevance(cmd.txnId(), cmd.saveStatus(), cmd.durability(), cmd.executeAtIfKnown(), cmd.participants().stillTouches());
        }

        public final Relevance relevance(TxnId txnId, @Nullable SaveStatus saveStatus, @Nullable Durability durability, @Nullable Timestamp executeAt, @Nullable Unseekables<?> participants)
        {
        ...
        private Relevance supersedingRelevance(TxnId txnId, @Nullable SaveStatus saveStatus, @Nullable Timestamp executeAt, @Nullable Unseekables<?> participants)
        {
            ...
            if (saveStatus != null)
            {
                switch (saveStatus.known.deps())
                {
                    ...
                    case DepsUnknown:
                        // SyncPoint do not collect additional dependencies in their Accept phase; to support this we must
                        ...
                        if (txnId.isSyncPoint() && saveStatus.is(Status.PreAccepted))
                            break;
                    case DepsErased:
                        return IRRELEVANT;
        ...
        public final <P> Summary get(Relevance relevance, TxnId txnId, Timestamp executeAt, SaveStatus saveStatus, Durability durability, Participants<?> touches, @Nullable P deps, TriPredicate<P, TxnId, Unseekables<?>> depTester)
        {
            ...
            switch (relevance)
            {
                ...
                case SUPERSEDING:
                    ...
                    else Invariants.require(txnId.isSyncPoint() && saveStatus.summary == PREACCEPTED);
                case ACTIVE:
            }
