    ...
    private static Cleanup cleanupWithFullRoute(Agent agent, boolean isPartial, StoreParticipants participants, TxnId txnId, SaveStatus saveStatus, Durability durability, RedundantBefore redundantBefore, DurableBefore durableBefore)
    {
        ...
        switch (redundant)
        {
            ...
            case SHARD_REDUNDANT:
                ...
                {
                    agent.onViolation(String.format("Loading decided and SHARD_REDUNDANT command (%s,%s) that has not been Applied. Expected to be witnessed and executed by %s.", txnId, saveStatus, redundantBefore.max(participants.route, e -> e.shardAppliedOrInvalidatedBefore)));
                    return TRUNCATE;
                }
            case WAS_OWNED:
        }
