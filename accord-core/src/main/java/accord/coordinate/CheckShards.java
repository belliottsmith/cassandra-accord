...
public abstract class CheckShards extends QuorumReadCoordinator<CheckStatusReply>
{
    ...
    protected CheckShards(Node node, TxnId txnId, RoutingKeys contactKeys, long untilRemoteEpoch, IncludeInfo includeInfo)
    {
        super(node, ensureSufficient(node, txnId, contactKeys, untilRemoteEpoch), txnId);
    ...
    protected void onDone(Done done, Throwable failure)
    {
        ...
        if (merged instanceof CheckStatusOkFull)
            merged = ((CheckStatusOkFull) merged).covering(contactKeys);
        ...
        node.ifLocal(scopeFor(txnId), merged.homeKey, txnId.epoch, store -> {
