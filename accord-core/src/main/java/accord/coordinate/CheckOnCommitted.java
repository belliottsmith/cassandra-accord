...
public class CheckOnCommitted extends CheckShards
{
    ...
    final AbstractRoute inform;

    CheckOnCommitted(Node node, TxnId txnId, AbstractRoute inform, long untilRemoteEpoch, long untilLocalEpoch, BiConsumer<? super CheckStatusOkFull, Throwable> callback)
    {
        this(node, txnId, inform, inform.with(inform.homeKey), untilRemoteEpoch, untilLocalEpoch, callback);
    }

    CheckOnCommitted(Node node, TxnId txnId, AbstractRoute inform, RoutingKeys contact, long untilRemoteEpoch, long untilLocalEpoch, BiConsumer<? super CheckStatusOkFull, Throwable> callback)
    {
        // TODO (soon): restore behaviour of only collecting info if e.g. Committed or Executed
        super(node, txnId, contact, untilRemoteEpoch, IncludeInfo.All);
        Preconditions.checkArgument(contact.contains(inform.homeKey));
        this.inform = inform;
        ...
        switch (full.fullStatus)
        {
            case Invalidated:
                AbstractRoute route = AbstractRoute.merge(route(), full.route);
                    // TODO (soon): shouldn't need to have loaded Command from disk to perform this (and other operations)
                node.forEachLocal(scopeFor(txnId), route, txnId.epoch, untilLocalEpoch, commandStore -> {
        ...
        switch (full.fullStatus)
        {
            ...
            case Applied:
                if (canExecute)
                {
                    ...
                    node.forEachLocal(scopeFor(txnId), maxRoute, txnId.epoch, untilLocalEpoch, commandStore -> {
                        Command command = commandStore.command(txnId);
