...
public class Invalidate implements Callback<InvalidateReply>
{
    ...
    public synchronized void onSuccess(Id from, InvalidateReply reply)
    {
        ...
        if (!reply.isOk())
        {
            ...
            {
                node.ifLocalSince(TxnOperation.scopeFor(txnId), invalidateWithKey, txnId, instance -> {
    ...
    private void invalidate()
    {
        // first look to see if it has already been
        {
            ...
            switch (maxStatus)
            {
                ...
                case Invalidated:
                    node.forEachLocalSince(TxnOperation.scopeFor(txnId), informKeys, txnId, instance -> {
