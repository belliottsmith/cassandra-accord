...
public class FindRoute extends CheckShards<Route<?>>
{
    public static class Result
    {
        ...
        public Result(CheckStatusOk ok)
        {
            ...
            this.executeAt = ok.maxKnown().isExecuteAtKnown() ? ok.executeAt : null;
        }
    }

    final BiConsumer<Result, Throwable> callback;
    FindRoute(Node node, SequentialAsyncExecutor executor, TxnId txnId, InvalidIf invalidIf, Route<?> someRoute, BiConsumer<Result, Throwable> callback)
    {
        super(node, executor, txnId, someRoute, IncludeInfo.Route, null, invalidIf);
        this.callback = callback;
    }

    public static FindRoute findRoute(Node node, SequentialAsyncExecutor executor, TxnId txnId, InvalidIf invalidIf, Route<?> someRoute, BiConsumer<Result, Throwable> callback)
    {
        FindRoute findRoute = new FindRoute(node, executor, txnId, invalidIf, someRoute, callback);
        findRoute.start();
        return findRoute;
    }
