...
public class SimpleProgressLog implements Runnable, ProgressLog.Factory
{
    ...
    static class BlockingState
    {
        ...
        void recordBlocking(Command command, Status blockedUntil, RoutingKeys blockedOnKeys)
        {
            ...
            node.withEpoch(srcEpoch, () -> {
                ...
                if (route == null || !route.covers(ranges))
                {
                    ...
                    BiConsumer<FindRoute.Result, Throwable> foundRoute = (findRoute, fail) -> {
                        ...
                        {
                            progress = Expected;
                            if (findRoute != null && findRoute.route != null && !(blockedOnKeys instanceof Route))
        ...
        {
            return AbstractRoute.merge(command.route(), blockedOnKeys instanceof AbstractRoute ? (AbstractRoute) blockedOnKeys : null);
        }
        ...
        private void invalidate(Node node, Command command)
        {
            ...
            if (someKey == null) someKey = blockedOnKeys.get(0);

            RoutingKeys informKeys = this.blockedOnKeys;
            if (informKeys == null) informKeys = route(command);
            if (informKeys == null) informKeys = RoutingKeys.of(someKey);
            else if (!informKeys.contains(someKey)) informKeys = informKeys.with(someKey);
            debugInvestigating = Invalidate.invalidate(node, command.txnId(), informKeys, someKey, (success, fail) -> {
    ...
    class Instance implements ProgressLog
    {
        ...
        public void waiting(TxnId blockedBy, Status blockedUntil, RoutingKeys blockedOnKeys)
        {
            ...
            if (!blockedByCommand.hasBeen(blockedUntil))
                ensure(blockedBy).recordBlocking(blockedByCommand, blockedUntil, blockedOnKeys);
        }
    }
