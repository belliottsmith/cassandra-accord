...
;            Invariants.checkState(globalCommand != null && !globalCommand.isEmpty());
    ...
    public static class InMemorySafeStore extends AbstractSafeCommandStore<InMemorySafeCommand, InMemorySafeTimestampsForKey, InMemorySafeCommandsForKey, InMemoryCommandStoreCaches>
    {
        ...
        public void updateExclusiveSyncPoint(Command prev, Command updated)
        {
            ...
            if (updated.txnId().kind() != Txn.Kind.ExclusiveSyncPoint || updated.txnId().domain() != Range || !updated.hasBeen(Applied) || prev.hasBeen(Applied)) return;
            ...
            for (Map.Entry<TxnId, GlobalCommand> entry : commandStore().commands.headMap(updated.txnId(), false).entrySet())
            {
                ...
                if (txnId.is(ExclusiveSyncPoint) && command.participants().owns().isEmpty()) continue; // we can execute not-owned RX out of order since they all logically have same dependencies
                Participants<?> intersecting = command.participants().executes().intersecting(covering, Minimal);
