...
public class CommandsForKeys
{
    ...
    public static Command.DurableAndIdempotentListener registerCommand(SafeTimestampsForKey timestamps, SafeCommandsForKey.Update<?,?> update, Command command)
    {
        ...
        update.common().byId().add(command.txnId(), command);
        update.common().byExecuteAt().add(command.txnId(), command);
    ...
    public static void registerNotWitnessed(AbstractSafeCommandStore<?,?,?,?> safeStore, RoutableKey key, TxnId txnId)
    {
        ...
        if (cfk.current().byId().commands.containsKey(txnId))
            return;

        tfk.updateMax(txnId);
        SafeCommandsForKey.Update<?,?> update = safeStore.getOrCreateCommandsForKeyUpdate(key);

        update.common().byId().add(txnId, uninitialised(txnId));
        update.common().byExecuteAt().add(txnId, uninitialised(txnId));
    }
    ...
    public static void listenerUpdate(AbstractSafeCommandStore<?,?,?,?> safeStore, RoutableKey listenerKey, Command command)
    {
        ...
        switch (command.status())
        {
            ...
            case ReadyToExecute:
                update.common().byId().add(command.txnId(), command);
                update.common().byExecuteAt().remove(command.txnId());
                update.common().byExecuteAt().add(command.executeAt(), command);
                break;
            case Invalidated:
                update.common().byId().remove(command.txnId());
                update.common().byExecuteAt().remove(command.txnId());
            case Truncated:
                break;
        }
