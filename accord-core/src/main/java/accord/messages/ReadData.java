...
public abstract class ReadData extends AbstractEpochRequest<ReadData.CommitOrReadNack> implements Command.TransientListener, EpochSupplier
{
    ...
    Ranges unavailable(SafeCommandStore safeStore, Command command)
    {
        if (txnId.kind().awaitsOnlyDeps())
        {
            Ranges stale = safeStore.commandStore().redundantBefore().staleRanges();
            return stale.intersecting(command.partialTxn().keys());
        }
        else
        {
            return safeStore.ranges().unsafeToReadAt(command.executeAt());
        }
    }
    ...
    {
        Invariants.checkState(waitingOn.get(commandStore.id()) && reading.get(commandStore.id()), "Txn %s's reading or waiting on does not contain store %d; waitingOn=%s; reading=%s", txnId, commandStore.id(), waitingOn, reading);
    ...
    protected void onOneSuccess(@Nullable Ranges newUnavailable)
    {
    ...
    {
        removeListener(safeStore, safeStore.ifInitialised(txnId));
    }

    private void removeListener(SafeCommandStore safeStore, SafeCommand safeCommand)
    {
