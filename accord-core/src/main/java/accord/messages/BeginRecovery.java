...
public class BeginRecovery extends TxnRequest<BeginRecovery.RecoverReply>
{
    ...
    private static Deps acceptedStartedBeforeWithoutWitnessing(SafeCommandStore safeStore, TxnId startedBefore, Ranges ranges, Seekables<?, ?> keys)
    {
        try (Deps.Builder builder = Deps.builder())
        {
            ...
            safeStore.mapReduce(keys, ranges, KeyHistory.ALL, shouldHaveWitnessed(startedBefore.rw()), STARTED_BEFORE, startedBefore, WITHOUT, startedBefore, Accepted, PreCommitted,
    ...
    private static Deps committedStartedBeforeAndWitnessed(SafeCommandStore safeStore, TxnId startedBefore, Ranges ranges, Seekables<?, ?> keys)
    {
        try (Deps.Builder builder = Deps.builder())
        {
            safeStore.mapReduce(keys, ranges, KeyHistory.ALL, shouldHaveWitnessed(startedBefore.rw()), STARTED_BEFORE, startedBefore, WITH, startedBefore, Committed, null,
                                          (p1, keyOrRange, txnId, executeAt, status, deps, prev) -> builder.add(keyOrRange, txnId), null, (Deps.AbstractBuilder<Deps>)builder, null);
            return builder.build();
        }
    }
    ...
    private static boolean hasAcceptedStartedAfterWithoutWitnessing(SafeCommandStore safeStore, TxnId startedAfter, Ranges ranges, Seekables<?, ?> keys)
    {
         ...
         */
        return safeStore.mapReduce(keys, ranges, KeyHistory.ALL, shouldHaveWitnessed(startedAfter.rw()), STARTED_AFTER, startedAfter, WITHOUT, startedAfter, Accepted, PreCommitted,
                                             (p1, keyOrRange, txnId, executeAt, status, deps, prev) -> true, null, false, true);
    }
    ...
    private static boolean hasCommittedExecutesAfterWithoutWitnessing(SafeCommandStore safeStore, TxnId startedAfter, Ranges ranges, Seekables<?, ?> keys)
    {
         ...
         */
        return safeStore.mapReduce(keys, ranges, KeyHistory.ALL, shouldHaveWitnessed(startedAfter.rw()), EXECUTES_AFTER, startedAfter, WITHOUT, startedAfter, Committed, null,
                                             (p1, keyOrRange, txnId, executeAt, status, deps, prev) -> true, null, false, true);
    }
}
